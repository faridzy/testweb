...
class NewsModel extends WCIFX_Model{
	function __construct(){
		parent::__construct();
	}
	function getListNews($limit = 10 , $where = array()){
		$where['limit'] = $limit;
		$data = $this->PostModel->getPosts($where);
		return $data;

	}
	function getDatatable(){
		$array = array("post_id", "title", "date_created", "author");
	    ...
	    $this->db->select("SQL_CALC_FOUND_ROWS post_id, title, date_created, author" ,FALSE);
	    ...
	    foreach ($q as $val) {
        $act = '<a href="'.site_url('admin/web-content/delete-post/news/'.$val->post_id).'" class="btn btn-danger">Delete</a>
